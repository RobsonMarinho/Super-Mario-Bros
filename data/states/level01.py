...
class Level1(tools._State):
    ...
    def startup(self, current_time, persist):
        """Chamado quando o objeto de estado é criado."""
    ...
    def setup_background(self):
        """Define a imagem de fundo, rect e dimensiona-lo para o correto
proporções"""
    ...
    def setup_ground(self):
        """Cria retângulos invisíveis, para os sprites andar sobre """
    ...
    def setup_pipes(self):
        """Criar collideable rects para todos os tubos"""
    ...
    def setup_steps(self):
        """Criar collideable rects para todas as etapas"""
    ...
    def setup_bricks(self):
        """Cria todos os tijolos frágeis para o nível. Moeda e
powerup grupos são criados para que eles podem ser passados para tijolos."""
    ...
    def setup_coin_boxes(self):
        """Cria todas as caixas de moeda e coloca-los em um grupo de sprite"""
    ...
    def setup_flag_pole(self):
        """Cria o pólo de bandeira no final do nível"""
    ...
    def setup_enemies(self):
        """Cria todos os inimigos e os armazena em uma lista de listas."""
    ...
    def setup_mario(self):
        """Coloca Mario no início do nível"""
        self.mario = mario.Mario()
        self.mario.rect.x = self.viewport.x + 110
        self.mario.rect.bottom = c.GROUND_HEIGHT

    def setup_checkpoints(self):
        """Cria postos de controle invisíveis que quando colidiu irá acionar
a criação de inimigos da self.enemy_group_list"""
    ...
    def setup_spritegroups(self):
        """Grupos de Sprite criados por conveniência"""
    ...
    def update(self, surface, keys, current_time):
        """Atualiza o nível inteiro usando Estados. Chamado pelo objeto de controle"""
    ...
    def handle_states(self, keys):
        """Se o nível está em um estado de congelados, apenas mario irá atualizar"""
    ...
    def update_during_transition_state(self, keys):
        """Atualiza o mario em um estado de transição (como tornar-se grande, pequeno,
ou morre). Verifica se ele deixa o estado de transição ou morre para
alterar o nível estado volta"""
    ...
    def check_if_mario_in_transition_state(self):
        """Se o mario está em um estado de transição, o nível será em um congelamento
estado"""
    ...
    def update_all_sprites(self, keys):
        """Atualiza a localização de todos os sprites na tela
."""
        self.mario.update(keys, self.game_info, self.powerup_group)
        for score in self.moving_score_list:
            score.update(self.moving_score_list, self.game_info)
    ...
    def check_points_check(self):
        """Detectar se a colisão de ponto de verificação ocorre, excluir ponto de verificação,
Adicionar inimigos para self.enemy_group"""
    ...
    def create_flag_points(self):
        """Cria os pontos que aparecem quando Mario toca o
mastro"""
        x = 8518
    ...
    def adjust_sprite_positions(self):
        """Ajusta os sprites por seus x e y velocidades e colisões"""
    ...
    def adjust_mario_position(self):
        """Ajusta a posição de Mario com base no seu x, y velocidades e
possíveis colisões"""
    ...
    def check_mario_x_collisions(self):
        """Verificação de colisões após Mario é movido no eixo x"""
    ...
    def convert_mushrooms_to_fireflowers(self):
        """Quando becomees Mario grandes, converte todos os powerups fireflower para
powerups do cogumelo"""
    ...
    def convert_fireflowers_to_mushrooms(self):
        """Quando Mario se torna pequeno, converte todos os cogumelos powerups para
fireflower powerups"""
        for brick in self.brick_group:
    ...
    def adjust_mario_for_x_collisions(self, collider):
        """Coloca o resplendor de Mario ao lado o colisor depois da mudança no eixo x
"""
    ...
    def adjust_mario_for_x_shell_collisions(self, shell):
        """Lida com Mario se ele bate uma concha movendo-se no eixo x"""
    ...
    def check_mario_y_collisions(self):
        """Verificações para colisões quando Mario se move ao longo do eixo y"""
    ...
    def prevent_collision_conflict(self, obstacle1, obstacle2):
        """Permite colisões somente para o item mais próximo do mario centerx"""
    ...
    def adjust_mario_for_y_coin_box_collisions(self, coin_box):
        """Colisões de Mario com caixas de moeda no eixo y"""
    ...
    def adjust_mario_for_y_brick_collisions(self, brick):
        """Colisões de Mario com tijolos no eixo y"""
    ...
    def check_if_enemy_on_brick(self, brick):
        """Mata o inimigo se sobre um tijolo quebrado ou colidido"""
        brick.rect.y -= 5
    ...
    def adjust_mario_for_y_ground_pipe_collisions(self, collider):
        """Colisões de Mario com tubos no eixo y"""
    ...
    def test_if_mario_is_falling(self):
        """Altera o Mario para um estado de queda, se mais do que um pixel acima um cachimbo,
chão, passo ou caixa"""
        self.mario.rect.y += 1
    ...
    def adjust_mario_for_y_enemy_collisions(self, enemy):
        """Colisões de Mario com todos os inimigos no eixo y"""
    ...
    def adjust_mario_for_y_shell_collisions(self, shell):
        """Colisões de Mario com Koopas em suas conchas no eixo y"""
        if self.mario.y_vel > 0:
            self.game_info[c.SCORE] += 400
            self.moving_score_list.append(
                score.Score(self.mario.rect.centerx - self.viewport.x,
    ...
    def adjust_enemy_position(self):
        """Move todos os inimigos ao longo do x, y-eixos e verificação de colisões"""
    ...
    def check_enemy_x_collisions(self, enemy):
        """Colisões de inimigos ao longo do eixo x. Remove o inimigo do inimigo grupo
para verificar contra todos os outros inimigos em seguida adiciona volta"""
        enemy.kill()
    ...
    def adjust_shell_position(self):
        """Se move qualquer koopa em um shell ao longo do x, y eixos e verifica se há
colisões"""
    ...
    def check_shell_x_collisions(self, shell):
        """Colisões de casca ao longo do eixo x"""
    ...
    def check_shell_y_collisions(self, shell):
        """Colisões de casca ao longo do eixo y"""
        collider = pg.sprite.spritecollideany(shell, self.ground_step_pipe_group)
    ...
    def adjust_powerup_position(self):
        """Move-se cogumelos, estrelas e bolas de fogo ao longo do x, y-eixos"""
    ...
    def adjust_mushroom_position(self, mushroom):
        """Cogumelo de movimentos ao longo do x, y-eixos"""
    ...
    def check_mushroom_x_collisions(self, mushroom):
        """Cogumelos colisões ao longo do eixo x"""
    ...
    def check_mushroom_y_collisions(self, mushroom):
        """Cogumelos colisões ao longo do eixo y"""
        collider = pg.sprite.spritecollideany(mushroom, self.ground_step_pipe_group)
        brick = pg.sprite.spritecollideany(mushroom, self.brick_group)
        coin_box = pg.sprite.spritecollideany(mushroom, self.coin_box_group)
    ...
    def adjust_mushroom_for_collision_x(self, item, collider):
        """Alterações do cogumelo direção se colisão ao longo eixo x"""
    ...
    def adjust_mushroom_for_collision_y(self, item, collider):
        """Alterações do cogumelo estado slide depois de bater o chão da queda"""
        item.rect.bottom = collider.rect.y
        item.state = c.SLIDE
        item.y_vel = 0

    def adjust_star_position(self, star):
        """Move-se invencível estrela ao longo de x, y eixos e verificações para colisões"""
    ...
    def check_star_y_collisions(self, star):
        """Invencíveis estrelas colisões ao longo do eixo y"""
    ...
    def adjust_star_for_collision_y(self, star, collider):
        """Permite um salto estrela do chão e na parte inferior de um caixa"""
    ...
    def adjust_fireball_position(self, fireball):
        """Move a bola de fogo ao longo de x, y-eixos e verificações para colisões"""
    ...
    def bounce_fireball(self, fireball):
        """Simula o ressalto de bola de fogo chão"""
        fireball.y_vel = -8
    ...
    def check_fireball_x_collisions(self, fireball):
        """Colisões da bola de fogo ao longo eixo x"""
    ...
    def check_fireball_y_collisions(self, fireball):
        """Colisões da bola de fogo ao longo do eixo y"""
        collide_group = pg.sprite.Group(self.ground_group,
                                        self.pipe_group,
                                        self.step_group,
                                        self.coin_box_group,
    ...
    def fireball_kill(self, fireball, enemy):
        """Mata o inimigo bater com a bola de fogo"""
        setup.SFX['kick'].play()
        self.game_info[c.SCORE] += 100
        self.moving_score_list.append(
            score.Score(enemy.rect.centerx - self.viewport.x,
    ...
    def check_if_falling(self, sprite, sprite_group):
        """Verifica se sprite deve entrar em um estado de queda"""
        sprite.rect.y += 1
    ...
    def delete_if_off_screen(self, enemy):
        """Remove o inimigo de grupos sprite se 500 pixels deixaram fora da tela,
por baixo da parte inferior da tela, ou direito da tela"""
    ...
    def set_game_info_values(self):
        """define os novos valores de jogos após a morte do jogador"""
    ...
    def check_if_time_out(self):
        """Verifique se o tempo tem atropelar a 0"""
    ...
    def update_while_in_castle(self):
        """Atualizações enquanto Mario estiver em castelo no final do nível"""
        for score in self.moving_score_list:
            score.update(self.moving_score_list, self.game_info)
    ...
    def update_flag_and_fireworks(self):
        """Atualiza o nível para os fogos de artifício e bandeira de castelo"""
        for score in self.moving_score_list:
            score.update(self.moving_score_list, self.game_info)
        self.overhead_info_display.update(self.game_info)
        self.flag_pole_group.update()
    ...
    def blit_everything(self, surface):
        """Insere sprites todos à superfície principal"""
